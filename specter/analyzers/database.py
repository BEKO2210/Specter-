...
klassischen, im Mittelstand haeufigen Fehler, ueber die Datenbanken kompromittiert
werden: oeffentlich erreichbare DB-Ports, fehlende Authentifizierung (typisch bei
Redis/MongoDB in Standardinstallationen), Standard-/Default-Zugangsdaten und
unverschluesselter Transport - rein offline, ohne Live-Verbindung, ohne Ausnutzung.
      ...
      "databases": [
        {
          ...
          "tls": false,              # ist der Transport verschluesselt?
...
Zugangsdaten, unsichere Transportverschluesselung).
"""
...
# Bekannte Datenbank-Standardports (nur fuer verstaendlichere Belege).
...
def _label(db: dict[str, Any]) -> tuple[str, str]:
    """Liefert (Anzeigename, Ort) fuer eine Datenbank aus engine/port."""
...
def _analyze_db(db: dict[str, Any]) -> list[Finding]:
    ...
    if db.get("public"):
        out.append(_mk(
            f"Datenbank oeffentlich erreichbar: {name}", "exposed_service",
            Severity.HOCH, loc,
            "Der Datenbank-Port ist aus fremden Netzen erreichbar - gehoert hinter "
            "Firewall/VPN, niemals offen ins Internet", cwe="CWE-668"))

    # auth_required kann explizit False sein; fehlt der Schluessel, nicht bewerten.
    ...
    if db.get("default_creds"):
        out.append(_mk(
            ...
            "Aktive Default-Zugangsdaten - sofort aendern; oeffentlich bekannt und "
            "erstes Angriffsziel", cwe="CWE-798"))

    if db.get("tls") is False:
        out.append(_mk(
            f"Unverschluesselter Datenbank-Transport: {name}", "transport_security",
            Severity.MITTEL, loc,
...
def analyze_database(data: dict[str, Any]) -> list[Finding]:
    """Fuehrt alle Datenbank-Expositionspruefungen aus und liefert die Findings."""
