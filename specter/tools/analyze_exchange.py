...
class AnalyzeExchangeTool:
    ...
    def spec(self) -> dict[str, Any]:
        return {
            ...
            "input_schema": {
                "type": "object",
                "properties": {
                    "path": {"type": "string", "description": "Pfad zur JSON-Datei (im Scope)."},
                },
                "required": ["path"],
            },
        }
    ...
    def run(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            self.audit.record("analyze_exchange.denied", path=raw_path, reason=str(exc))
        ...
        try:
            ...
            self.audit.record("analyze_exchange.parse_error", path=str(path),
                              reason=str(exc))
        ...
        self.audit.record("analyze_exchange.ok", path=str(path),
                          findings=len(findings), recorded=recorded)
        if not findings:
            return ToolResult("Exchange-Analyse ohne Befunde (oder unbekannte Struktur).")
        lines = [f"Exchange-Analyse: {len(findings)} Finding(s), {recorded} neu erfasst:"]
