...
class AnalyzeDnsTool:
    ...
    def run(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            self.audit.record("analyze_dns.denied", path=raw_path, reason=str(exc))
        ...
        try:
            ...
            self.audit.record("analyze_dns.parse_error", path=str(path), reason=str(exc))
            return ToolResult(f"Konnte JSON nicht lesen: {exc}", is_error=True)

        findings = analyze_dns(data)
        recorded = self.state.findings.extend(findings)
        self.audit.record("analyze_dns.ok", path=str(path),
                          findings=len(findings), recorded=recorded)
        if not findings:
            return ToolResult("DNS-Sicherheitsanalyse ohne Befunde (oder unbekannte Struktur).")
        lines = [f"DNS-Sicherheitsanalyse: {len(findings)} Finding(s), {recorded} neu erfasst:"]
