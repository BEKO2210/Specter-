...
class AnalyzeAdTool:
    ...
    def run(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            self.audit.record("analyze_ad.denied", path=raw_path, reason=str(exc))
        ...
        try:
            ...
            self.audit.record("analyze_ad.parse_error", path=str(path), reason=str(exc))
        ...
        self.audit.record("analyze_ad.ok", path=str(path),
                          findings=len(findings), recorded=recorded)
        if not findings:
            return ToolResult("AD-Analyse ohne Befunde (oder unbekannte Struktur).")
        lines = [f"AD-Analyse: {len(findings)} Finding(s), {recorded} neu erfasst:"]
