...
class AnalyzeEmailSecurityTool:
    ...
    def run(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            self.audit.record("analyze_email_security.denied", path=raw_path, reason=str(exc))
        ...
        try:
            ...
            self.audit.record("analyze_email_security.parse_error", path=str(path), reason=str(exc))
            return ToolResult(f"Konnte JSON nicht lesen: {exc}", is_error=True)

        findings = analyze_email_security(data)
        recorded = self.state.findings.extend(findings)
        self.audit.record("analyze_email_security.ok", path=str(path),
                          findings=len(findings), recorded=recorded)
        if not findings:
            return ToolResult("E-Mail-Security-Analyse ohne Befunde (oder unbekannte Struktur).")
        lines = [f"E-Mail-Security-Analyse: {len(findings)} Finding(s), {recorded} neu erfasst:"]
