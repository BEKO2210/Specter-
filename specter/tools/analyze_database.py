...
class AnalyzeDatabaseTool:
    ...
    def run(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            self.audit.record("analyze_database.denied", path=raw_path, reason=str(exc))
        ...
        try:
            ...
            self.audit.record("analyze_database.parse_error", path=str(path), reason=str(exc))
            return ToolResult(f"Konnte JSON nicht lesen: {exc}", is_error=True)

        findings = analyze_database(data)
        recorded = self.state.findings.extend(findings)
        self.audit.record("analyze_database.ok", path=str(path),
                          findings=len(findings), recorded=recorded)
        if not findings:
            return ToolResult("Datenbank-Analyse ohne Befunde (oder unbekannte Struktur).")
        lines = [f"Datenbank-Analyse: {len(findings)} Finding(s), {recorded} neu erfasst:"]
