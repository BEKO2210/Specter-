...
class AnalyzeEntraTool:
    ...
    def spec(self) -> dict[str, Any]:
        return {
            "name": self.name,
            "description": (
                "Analysiert einen bereitgestellten Entra-ID-/Microsoft-365-Export "
    ...
    def run(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            self.audit.record("analyze_entra.denied", path=raw_path, reason=str(exc))
        ...
        try:
            ...
            self.audit.record("analyze_entra.parse_error", path=str(path), reason=str(exc))
        ...
        self.audit.record("analyze_entra.ok", path=str(path),
                          findings=len(findings), recorded=recorded)
        if not findings:
            return ToolResult("Entra-ID-/M365-Analyse ohne Befunde (oder unbekannte Struktur).")
        lines = [f"Entra-ID-/M365-Analyse: {len(findings)} Finding(s), {recorded} neu erfasst:"]
